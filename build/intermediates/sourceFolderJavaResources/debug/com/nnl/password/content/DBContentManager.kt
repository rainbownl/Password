...
import com.nnl.password.encrypt.MyEncryptor
...
class DBContentManager private constructor(context: Context) : IDataBaseManager{
    ...
    var encryptor: MyEncryptor = MyEncryptor()
    ...
    fun readLicense() : String? {
        ...
        if (cursor != null && cursor!!.count > 0) {
            ...
            var license = cursor?.getBlob(0)
            cursor.close()
            if (license != null) {
                try {
                    return String(encryptor.decrypt(license)!!)
                }catch (e:Exception){
                    e.printStackTrace()
                }
                //return license
    ...
    fun writeLicense(license: String) {
        if (this.license == null) {
            var columes = ContentValues()
            var byteArray = encryptor.encrypt(license.toByteArray())
            columes.put(DBDefines.COLUME_LICENSE, byteArray)
            //columes.put(DBDefines.COLUME_LICENSE, license)
            databaseManager?.deleteByWhere(DBDefines.LICENSE_TABLE_NAME, "1=1", null)
            databaseManager?.insert(DBDefines.LICENSE_TABLE_NAME, columes)
        } else {
            var values = ContentValues()
            values.put(DBDefines.COLUME_LICENSE, encryptor.encrypt(license.toByteArray()))
